...
class LinearReservoir(BaseReservoir):
    ...
    def initialize_ranked_storage(self):
        """
        Computes the initial ranked storage and "old" storage based on the
        solution of the steady-state eqation (assuming average input fluxes)
        or randomly sampled reservoirs. The transit time distribution (TTD) is
        obtained by convolving the local TTD since entry in the reservoir with
        the TTD of the input flux.
        ...
        if self.initial_TTD is not None:
            ...
            S_bar = IN_bar / self._parameters['a']

            local_steady_state_TTD = (
                self._parameters['a'] / S_bar
                * np.exp(-self._parameters['a'] / S_bar * np.arange(self.num_ages_tracked))
            )
